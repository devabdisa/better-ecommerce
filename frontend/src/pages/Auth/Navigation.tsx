...
const Navigation: FC = () => {
  ...
  return (
    <>
      ...
      <div
        style={{ zIndex: 9999 }}
        className="hidden md:flex flex-col justify-between p-4 text-white bg-[#0f0f10] border-r border-[#27272a] w-[5%] lg:w-[4%] hover:w-[15%] h-screen fixed transition-all duration-300 ease-in-out shadow-[4px_0_24px_rgba(0,0,0,0.5)] group z-50"
      ...
      >
        <div className="flex flex-col justify-center space-y-4 pt-4">
          <Link
            ...
            className={`flex items-center transition-all transform hover:translate-x-2 p-3 rounded-xl hover:bg-[#27272a] group/item ${
              isActive("/") ? "bg-[#27272a] text-primary" : ""
            }`}
          >
            <AiOutlineHome
          ...
          <Link
            to="/shop"
            className={`flex items-center transition-all transform hover:translate-x-2 p-3 rounded-xl hover:bg-[#27272a] group/item ${
              isActive("/shop") ? "bg-[#27272a] text-primary" : ""
            }`}
          >
            <AiOutlineShopping
              className={`shrink-0 mr-2 ${
          ...
          <Link
            ...
            className={`flex items-center transition-all transform hover:translate-x-2 p-3 rounded-xl hover:bg-[#27272a] group/item relative ${
              isActive("/cart") ? "bg-[#27272a] text-primary" : ""
            }`}
          >
          ...
          <Link
            to="/favorite"
            className={`flex items-center transition-all transform hover:translate-x-2 p-3 rounded-xl hover:bg-[#27272a] group/item relative ${
              isActive("/favorite") ? "bg-[#27272a] text-primary" : ""
            }`}
          >
            <div className="flex items-center relative">
              <FaHeart
                className={`shrink-0 mr-2 ${
                  isActive("/favorite") ? "text-pink-500" : "text-gray-400"
                } group-hover/item:text-pink-500 transition-colors`}
                size={24}
              />
              {favorites.length > 0 && (
          ...
          {userInfo && (
            <Link
              to="/user-orders"
              className={`flex items-center transition-all transform hover:translate-x-2 p-3 rounded-xl hover:bg-[#27272a] group/item relative ${
                isActive("/user-orders") ? "bg-[#27272a] text-primary" : ""
              }`}
            >
              <div className="flex items-center relative">
                <AiOutlineInbox
                  className={`shrink-0 mr-2 ${
                    isActive("/user-orders") ? "text-blue-500" : "text-gray-400"
        ...
        <div className="relative mb-5" ref={dropdownRef}>
          ...
          {dropdownOpen && userInfo && (
            <div className="absolute bottom-full left-0 w-60 ml-2 mb-4 bg-[#18181b] text-gray-200 border border-[#27272a] rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.8)] overflow-hidden z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
              {userInfo.isAdmin && (
          ...
          {!userInfo && (
            <ul className="space-y-4">
              <li>
                <Link
                  to="/login"
                  className="flex items-center transition-transform transform hover:translate-x-2 p-2 rounded-xl hover:bg-white/5 group/item"
                >
                  <AiOutlineLogin
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0f0f10]/80 backdrop-blur-xl border-t border-[#27272a] z-50 flex items-center justify-around p-2 pb-6">
        <Link
          to="/"
        ...
        <Link
          to="/shop"
          className={`flex flex-col items-center p-2 rounded-xl transition-all ${
            isActive("/shop") ? "text-primary scale-110" : "text-gray-400"
          }`}
        >
        ...
        <Link
          ...
          className={`flex flex-col items-center p-2 rounded-xl transition-all relative ${
            isActive("/cart") ? "text-primary scale-110" : "text-gray-400"
          }`}
        >
          <AiOutlineShoppingCart size={24} />
          {cartItems.length > 0 && (
            <span className="absolute top-1 right-1 px-1.5 py-0.5 text-[8px] font-bold text-white bg-blue-600 rounded-full border border-[#0f0f10]">
              {cartItems.reduce((a, c) => a + c.qty, 0)}
            </span>
          )}
        ...
        <Link
          to="/favorite"
          className={`flex flex-col items-center p-2 rounded-xl transition-all relative ${
            isActive("/favorite") ? "text-pink-500 scale-110" : "text-gray-400"
          }`}
        >
          <FaHeart size={22} />
          {favorites.length > 0 && (
            <span className="absolute top-1 right-1 px-1.5 py-0.5 text-[8px] font-bold text-white bg-pink-500 rounded-full border border-[#0f0f10]">
              {favorites.length}
            </span>
          )}
          <span className="text-[10px] font-bold mt-1">Liked</span>
        </Link>
        ...
        {userInfo ? (
          <Link
            to="/profile"
            className={`flex flex-col items-center p-2 rounded-xl transition-all ${
              isActive("/profile") ? "text-blue-500 scale-110" : "text-gray-400"
            }`}
          >
            <div className="w-6 h-6 rounded-full bg-linear-to-tr from-blue-600 to-purple-600 flex items-center justify-center text-[10px] text-white font-bold border border-white/10">
              {userInfo.username.substring(0, 1).toUpperCase()}
            </div>
          ...
          <Link
            to="/login"
            className={`flex flex-col items-center p-2 rounded-xl transition-all ${
              isActive("/login") ? "text-white scale-110" : "text-gray-400"
            }`}
          >
